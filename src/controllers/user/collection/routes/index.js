    ...
    try {
      const [countResult] = await sequelize.query(`
        SELECT count(*) as count FROM(
          ...
          INNER JOIN plays AS play ON (play.tid = track.tid AND play.event = 1 AND play.uid = :listenerId)
          WHERE track.status IN (0, 2, 3)
          AND track.track_cover_art != ''
          GROUP BY track.tid, um.meta_value
          HAVING COUNT(DISTINCT play.pid) >= 9
        ) as count
      `, {
      ...
      const result = await sequelize.query(`
        SELECT track.tid as id, track.uid as creator_id, track.status, track.track_name, track.track_url, track.track_cover_art as cover_art, trackgroup.cover as cover, file.id as file, track.track_album, track.track_duration, meta.meta_value as artist, count(play.pid) as count
